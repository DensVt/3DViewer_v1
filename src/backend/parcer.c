...
void find_max_min(main_data *data) {
  ...
  for (unsigned int i = 0; i < data->count_ver; i++) {
    if (fabs(data->arr_ver->matrix[i][0]) > data->max_X)
      data->max_X = fabs(data->arr_ver->matrix[i][0]);
    if (fabs(data->arr_ver->matrix[i][1]) > data->max_Y)
      data->max_Y = fabs(data->arr_ver->matrix[i][1]);
    if (fabs(data->arr_ver->matrix[i][2]) > data->max_Z)
      data->max_Z = fabs(data->arr_ver->matrix[i][2]);
    if (fabs(data->arr_ver->matrix[i][0]) < data->min_X)
      data->min_X = fabs(data->arr_ver->matrix[i][0]);
    if (fabs(data->arr_ver->matrix[i][1]) < data->min_Y)
      data->min_Y = fabs(data->arr_ver->matrix[i][1]);
    if (fabs(data->arr_ver->matrix[i][2]) < data->min_Z)
      data->min_Z = fabs(data->arr_ver->matrix[i][2]);
  }
}
...
void v_parcer(main_data *data,
              char *str) {  // парсим в структуру координаты вершин
  ...
  if (s21_create_matrix(1, size, &data->poligons[data->index_f]) == 0 &&
    ...
    for (int i = 1; str[i] != '\0'; i++) {
      if (isint(str[i]) && slesh_flag == 0) {
        ...
        if (num < 0) {
          num *= -1;
          num = data->index_v + 1 - num;
        }
        if (num > (int)data->count_ver) {
          data->correct_obj = 0;
        }
...
void check_str_state(
    char *str, main_data *data) {  // проверяем вершина или порядок (v ? f)
  if (str[0] == 'v' && str[1] == ' ') {
    v_parcer(data, str);
    data->index_v++;
  } else if (str[0] == 'f') {
    parcer_f(data, str);
    data->index_f += 1;
  } else if ((str[0] == 'v' || str[0] == 'f') && str[1] == '\0')
    data->correct_obj = 0;
  else if ((str[0] == 'v' || str[0] == 'f') && str[1] == '\n')
    data->correct_obj = 0;
}
...
int parcer(char *path_to_file, main_data *data) {  // основная функция парсинга
  ...
  if (file) {
    ...
    if (data->count_ver > 0 && data->count_edge > 0 && data->arr_ver &&
        data->poligons) {
...
double grad_to_rad(double grad) { return (grad * PI) / (double)180; }
// rotate around z
void rot_z(main_data *data, double angle) {
  ...
  for (int i = 0; i < (int)data->count_ver; i++) {
    double x = data->arr_ver->matrix[i][0] * cos(angle_rad) +
               data->arr_ver->matrix[i][1] * sin(angle_rad) +
               data->arr_ver->matrix[i][2] * 0;
    double y = data->arr_ver->matrix[i][0] * (-sin(angle_rad)) +
               data->arr_ver->matrix[i][1] * cos(angle_rad) +
               data->arr_ver->matrix[i][2] * 0;
    double z = data->arr_ver->matrix[i][0] * 0 +
               data->arr_ver->matrix[i][1] * 0 +
               data->arr_ver->matrix[i][2] * 1;
...
void rot_y(main_data *data, double angle) {
  ...
  for (int i = 0; i < (int)data->count_ver; i++) {
    double x = data->arr_ver->matrix[i][0] * cos(angle_rad) +
               data->arr_ver->matrix[i][1] * 0 +
               data->arr_ver->matrix[i][2] * sin(angle_rad);
    double y = data->arr_ver->matrix[i][0] * 0 +
               data->arr_ver->matrix[i][1] * 1 +
               data->arr_ver->matrix[i][2] * 0;
    double z = data->arr_ver->matrix[i][0] * (-sin(angle_rad)) +
               data->arr_ver->matrix[i][1] * 0 +
               data->arr_ver->matrix[i][2] * cos(angle_rad);
    data->arr_ver->matrix[i][0] = x;
    data->arr_ver->matrix[i][1] = y;
    data->arr_ver->matrix[i][2] = z;
  }
...
void rot_x(main_data *data, double angle) {
  ...
  for (int i = 0; i < (int)data->count_ver; i++) {
    double x = data->arr_ver->matrix[i][0] * 1 +
               data->arr_ver->matrix[i][1] * 0 +
               data->arr_ver->matrix[i][2] * 0;
    double y = data->arr_ver->matrix[i][0] * 0 +
               data->arr_ver->matrix[i][1] * cos(angle_rad) +
...
//     parcer("/Users/artemefimov/Documents/school-21/3d_restart/C8_3DViewer_v1.0-2/src/files_obj/text.txt",
//     &data);
//     // printf("e%d %d\n", data.correct_obj, data.count_ver);
//     // output(&data.arr_ver);
